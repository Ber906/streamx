// ─── Anti-Scrape Middleware ───────────────────────────────────────────────────
// Protection layers:
//   1. Rate limiting  — max requests per IP per window (in-memory, no extra deps)
//   2. Bot filtering  — block known scraper / bot user-agents
//   3. Origin check   — API routes must come from the same site (or have a valid Bearer token)
//   4. Security headers — adds common hardening headers on every response
//
// Scrapers / bots get Rick Roll'd instead of a boring error. 🎵
...
// ── Rick Roll response ────────────────────────────────────────────────────────
// For browser requests → full HTML redirect page with autoplay embed.
// For headless / API clients → JSON with a "helpful" link + 302 redirect.
function rickRoll(req, res) {
  ...
  if (isHeadless) {
    // Headless scrapers get a JSON "gift" and a Location header
    res.statusCode = 302;
  ...
  // Browser bots get a full-page Rick Roll
  res.statusCode = 200;
...
const RATE_LIMIT_MAX       = 80;        // requests per window per IP
...
// ── 3. Origin / Referer check for /api routes ─────────────────────────────────
...
function antiScrapeMiddleware(req, res, next) {
  ...
  if (limited) {
    // Too many requests → Rick Roll
    return rickRoll(req, res);
  }

  // Static assets don't need the deeper checks
  const isStaticAsset = /\.(js|css|png|jpg|jpeg|gif|svg|ico|woff2?|ttf|eot|map)$/i.test(url);
  if (isStaticAsset) return next();

  // Bot detected → Rick Roll
  if (isBot(ua)) {
    return rickRoll(req, res);
  }

  // Origin check on /api routes
  const isApiRoute = url.startsWith("/api");
  if (isApiRoute) {
    const sub = url.replace(/^\/api\/?/, "").split("?")[0].toLowerCase();
    const exempted = sub === "health" || sub.startsWith("auth/");
    if (!exempted && !hasValidOrigin(req)) {
      // Unauthorized API scraper → Rick Roll
      return rickRoll(req, res);
    }
  }
